...
# If Streamlit secrets contain a Google service account + sheet_id, entries
# persist permanently to a Google Sheet. Otherwise falls back to the local
# data/entries.json file (fine for testing, but resets on redeploy/sleep).
USE_SHEETS = "gcp_service_account" in st.secrets and "sheet_id" in st.secrets
ENTRY_COLS = ["date","tasks","goals","hours","status","notes","learning","idea","reflection","private","managerNote"]
...
def load_entries():
    if USE_SHEETS:
        sh = _gsheet()
        ws = _get_or_create_ws(sh, "entries", ENTRY_COLS)
        records = ws.get_all_records()
        if not records:
            for iso in sorted(SEED.keys()):
                e = SEED[iso]
                ws.append_row([e.get(c, "") for c in ENTRY_COLS])
            records = ws.get_all_records()
        entries = {}
        for r in records:
            iso = str(r.get("date", "")).strip()
            if not iso:
                continue
            e = {c: r.get(c, "") for c in ENTRY_COLS}
            e["hours"] = float(r.get("hours") or 0)
            e["private"] = str(r.get("private")).strip().lower() in ("true", "1", "yes")
            entries[iso] = e
        return entries
...
def save_entries(entries):
    if USE_SHEETS:
        ...
        ws = _get_or_create_ws(sh, "entries", ENTRY_COLS)
        ws.clear()
        ws.append_row(ENTRY_COLS)
        rows = [[entries[iso].get(c, "") for c in ENTRY_COLS] for iso in sorted(entries.keys())]
        if rows:
            ws.append_rows(rows)
        return
...
def load_notes():
    if USE_SHEETS:
        sh = _gsheet()
        ws = _get_or_create_ws(sh, "manager_notes", ["ts", "text"])
        return [{"ts": r.get("ts",""), "text": r.get("text","")} for r in ws.get_all_records()]
...
def save_notes(notes):
    if USE_SHEETS:
        ...
        ws = _get_or_create_ws(sh, "manager_notes", ["ts", "text"])
        ws.clear()
        ws.append_row(["ts", "text"])
        rows = [[n["ts"], n["text"]] for n in notes]
        if rows:
            ws.append_rows(rows)
        return
...
# ── Sidebar navigation ───────────────────────────────────────────────────
# ?view=manager in the URL opens straight into Manager View — this is the
# link to send your manager, e.g. https://dailytasklog.streamlit.app/?view=manager
_qp = st.query_params
_default_idx = 1 if _qp.get("view") == "manager" else 0

st.sidebar.markdown("### 🛰️ Field Log")
st.sidebar.caption("Network Success Operations · TraceLink")
view = st.sidebar.radio(
    "View", ["📝 My Log", "👩‍💼 Manager View"],
    index=_default_idx, label_visibility="collapsed"
)
st.sidebar.divider()
if _default_idx == 1:
    st.sidebar.info("Opened via the manager link. Share:\n\ndailytasklog.streamlit.app/?view=manager")
else:
...
if view == "📝 My Log":
    ...
    with tab1:
        with st.container(border=True):
            ...
            tasks = st.text_area("Tasks completed", value=e.get("tasks",""), placeholder="- Reviewed and sent 6 T18 documents\n- Attended Partner Scorecard sync")
...
# ══════════════════════════════════════════════════════════════════════
    ...
    st.write("")
    st.markdown("### Leave a comment on a specific day")
    with st.container(border=True):
    ...
    with st.container(border=True):
        new_note = st.text_area("Add a standalone note (not tied to a specific day)", key="general_note")
