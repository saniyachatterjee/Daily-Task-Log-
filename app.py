...
def normalize_goals(value):
    return normalize_tasks(value)


def goals_text(goals):
    return "\n".join(f"- {goal}" for goal in normalize_goals(goals))
...
def normalize_entries(entries):
    ...
    for iso, entry in entries.items():
        ...
        goals = normalize_goals(entry.get("goals"))
        if entry.get("tasks") != tasks:
            entry["tasks"] = tasks
            changed = True
        if entry.get("goals") != goals:
            entry["goals"] = goals
            changed = True
...
def load_entries():
    """Load from Supabase when configured; otherwise keep the original JSON fallback."""
    if SUPABASE:
        try:
            rows = SUPABASE.table("entries").select("*").order("entry_date").execute().data or []
            entries = {}
            for row in rows:
                entries[row["entry_date"]] = {
                    "date": row["entry_date"],
                    "tasks": normalize_tasks(row.get("tasks") or []),
                    "goals": normalize_goals(row.get("goals") or ""),
...
def save_entry(entry):
    ...
    if SUPABASE:
        try:
            SUPABASE.table("entries").upsert({
                "entry_date": entry["date"],
                "tasks": entry["tasks"],
                "goals": goals_text(entry.get("goals", [])),
...
def load_notes():
    if SUPABASE:
        try:
            rows = SUPABASE.table("manager_notes").select("*").order("created_at", desc=True).execute().data or []
            return [{
                "id": str(r.get("id", "")),
                "week_start": r.get("week_start", ""),
                "text": r.get("text", ""),
                "author": r.get("author", "Manager"),
...
def add_manager_note(week_start, author, text):
    if SUPABASE:
        try:
            SUPABASE.table("manager_notes").insert({
            ...
            st.error(f"Could not save manager note online: {exc}")
            return
    notes = load_notes()
    notes.append({
...
def render_entry(entries):
    ...
    existing_goals = normalize_goals(existing.get("goals"))
    if "goal_rows" not in st.session_state or st.session_state.get("goal_date") != iso:
        st.session_state.goal_rows = existing_goals[:] if existing_goals else [""]
        st.session_state.goal_date = iso

    st.markdown("**Goals for next day**")
    st.caption("One goal per line, with separate inputs for a clean bullet list.")
    remove_goal_idx = None
    for i in range(len(st.session_state.goal_rows)):
        c1, c2, c3 = st.columns([0.06, 0.84, 0.10])
        with c1:
            st.markdown(f'<div class="task-number">{i+1:02}</div>', unsafe_allow_html=True)
        with c2:
            st.session_state.goal_rows[i] = st.text_input("", value=st.session_state.goal_rows[i], key=f"goal_{iso}_{i}", placeholder="e.g. Complete remaining Litmos courses", label_visibility="collapsed")
        with c3:
            if len(st.session_state.goal_rows) > 1 and st.button("✕", key=f"remove_goal_{iso}_{i}"):
                remove_goal_idx = i
    if remove_goal_idx is not None:
        st.session_state.goal_rows.pop(remove_goal_idx)
        st.rerun()
    if st.button("＋ Add goal", key=f"add_goal_{iso}"):
        st.session_state.goal_rows.append("")
        st.rerun()
    c1,c2 = st.columns(2)
    with c1:
    ...
    if st.button("Save entry", type="primary", use_container_width=True):
        ...
        clean_goals = [x.strip() for x in st.session_state.goal_rows if x.strip()]
        save_entry({"date":iso,"tasks":clean_tasks,"goals":clean_goals,"hours":hours,"status":status,"notes":notes,"learning":learning,"idea":idea,"reflection":reflection,"private":private})
...
def _render_manager_content(entries, notes):
    ...
    st.caption("Live view · checks for new saved entries every 15 seconds")
    st.write("")
    render_stats(entries)
    with st.expander("Downloads"):
        render_downloads(entries, notes, key_prefix="manager")
    with st.expander("Manager Guide"):
        st.markdown("**How to use this dashboard**")
        st.markdown("1. Open the manager link.\n2. Review the latest week first.\n3. Check tasks, goals, hours and status.\n4. Use the note box to leave feedback.\n5. Use Downloads → spreadsheet when you need the full log.")
    if not entries:
...
def render_manager(entries=None, notes=None):
    # Re-read the database on each refresh so the manager sees new saved entries.
    entries = load_entries()
    notes = load_notes()
    _render_manager_content(entries, notes)
...
def build_word(entries, ws):
    ...
    for d in dates:
        ...
        row[2].text=goals_text(e.get("goals")) or "—"
...
def build_xlsx(entries, notes):
    ...
    for iso in sorted(entries):
        e = entries[iso]
        ws.append([iso, day_name(iso), tasks_text(e.get("tasks")), goals_text(e.get("goals", [])), e.get("hours", 0), (e.get("status") or "on-track").replace("-", " ").title(), e.get("notes", ""), e.get("learning", ""), e.get("idea", ""), e.get("reflection", "")])
...
def render_downloads(entries, notes, key_prefix="intern"):
    st.subheader("Downloads")
    xlsx = build_xlsx(entries, notes)
    st.download_button("⬇ Download full spreadsheet", data=xlsx, file_name="Field_Log_Full.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key=f"xlsx_full_{key_prefix}")
    weeks=sorted({week_start(d) for d in entries}, reverse=True)
    for ws in weeks:
        bio=build_word(entries,ws)
        st.download_button(f"⬇ Word report — week of {fmt_date(ws)}", data=bio, file_name=f"Field_Log_Week_{ws}.docx", mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document", key=f"word_{key_prefix}_{ws}")
...
def main():
    ...
    st.caption("Storage: " + ("Supabase (persistent)" if SUPABASE else "Local JSON (use Supabase for cloud persistence)"))
