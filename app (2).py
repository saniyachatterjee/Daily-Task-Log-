...
# ---------------------------------------------------------------------------
# Google Sheets backend (preferred, persistent). Falls back automatically to
# local JSON files if secrets aren't configured, so the app never breaks.
# ---------------------------------------------------------------------------
ENTRIES_HEADER = ["date", "tasks", "goals", "hours", "status", "notes", "learning", "idea", "reflection", "private"]
NOTES_HEADER = ["id", "date", "text", "author", "ts"]
...
@st.cache_resource(show_spinner=False)
def _gsheets_workbook():
    return _gspread_client().open_by_key(st.secrets["gsheet_id"])


def _gsheets_ws(name, header):
    wb = _gsheets_workbook()
    try:
        ws = wb.worksheet(name)
    except Exception:
        ws = wb.add_worksheet(title=name, rows=2000, cols=len(header) + 2)
        ws.append_row(header)
    return ws


@st.cache_data(ttl=4, show_spinner=False)
def _gsheets_load_entries():
    ws = _gsheets_ws("Entries", ENTRIES_HEADER)
    entries = {}
    for r in ws.get_all_records():
        ...
        if not d:
            continue
        try:
            tasks = json.loads(r.get("tasks") or "[]")
        except Exception:
            tasks = normalize_tasks(r.get("tasks"))
        entries[d] = {
            "date": d,
            "tasks": tasks,
            "goals": r.get("goals", ""),
            ...
            "private": str(r.get("private", "True")).strip().lower() in ("true", "1", "yes"),
        }
    return entries


def _gsheets_save_entry(entry):
    ws = _gsheets_ws("Entries", ENTRIES_HEADER)
    entry = dict(entry)
    entry["tasks"] = normalize_tasks(entry.get("tasks"))
    row = [
        entry["date"],
        json.dumps(entry["tasks"], ensure_ascii=False),
        entry.get("goals", ""),
        entry.get("hours", 0),
        entry.get("status", "on-track"),
        entry.get("notes", ""),
        entry.get("learning", ""),
        entry.get("idea", ""),
        entry.get("reflection", ""),
        str(bool(entry.get("private", True))),
    ]
    values = ws.get_all_values()
    target = None
    for i, r in enumerate(values[1:], start=2):
        if r and r[0] == entry["date"]:
            target = i
            break
    if target:
        ws.update(f"A{target}:J{target}", [row])
    else:
        ws.append_row(row)
    _gsheets_load_entries.clear()


@st.cache_data(ttl=4, show_spinner=False)
def _gsheets_load_notes():
    ws = _gsheets_ws("Notes", NOTES_HEADER)
    notes = []
    for r in ws.get_all_records():
        if not r.get("id"):
            continue
        notes.append({"id": r["id"], "date": r.get("date", ""), "text": r.get("text", ""),
                      "author": r.get("author", "Manager"), "ts": r.get("ts", "")})
    return notes


def _gsheets_add_note(day_iso, author, text):
    ws = _gsheets_ws("Notes", NOTES_HEADER)
    ws.append_row([
        datetime.now().strftime("%Y%m%d%H%M%S%f"),
        day_iso,
        text.strip(),
        author.strip() or "Manager",
        datetime.now().strftime("%Y-%m-%d %H:%M"),
    ])
    _gsheets_load_notes.clear()


def load_entries():
    if _gsheets_enabled():
        try:
            entries = _gsheets_load_entries()
            if not entries:
                for iso, e in SEED.items():
                    _gsheets_save_entry(e)
                entries = dict(SEED)
            entries, _ = normalize_entries(entries)
            return entries
        except Exception as e:
            st.warning(f"Couldn't reach Google Sheets ({e}). Showing local data instead.")
...
def save_entry(entry):
    ...
    if _gsheets_enabled():
        try:
            _gsheets_save_entry(entry)
            return
        except Exception as e:
            st.error(f"Couldn't save to Google Sheets ({e}). Saved locally instead — let Claude know so this can be fixed.")
    entries = load_json(ENTRIES_FILE, {}) if os.path.exists(ENTRIES_FILE) else dict(SEED)
...
def add_manager_note(day_iso, author, text):
    if _gsheets_enabled():
        try:
            _gsheets_add_note(day_iso, author, text)
            return
        except Exception as e:
            st.error(f"Couldn't save note to Google Sheets ({e}). Saved locally instead.")
